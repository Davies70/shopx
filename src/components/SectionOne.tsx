...
const SectionOne = () => {
  ...
  const translateY1 = useTransform(
    scrollYProgress,
    [0, 0.8, 1],
    [30, 0, -30] // Starts above, meets in middle, goes below
  );

  const translateY2 = useTransform(
    scrollYProgress,
    [0, 0.8, 1],
    [-30, 0, 30] // Starts below, meets in middle, goes above
  );

  const springY1 = useSpring(translateY1, { stiffness: 80, damping: 30 });
  const springY2 = useSpring(translateY2, { stiffness: 80, damping: 30 });

  return (
    <section
      ref={sectionRef}
      className='flex py-[72px] min-[479px]:py-[80px] min-[768px]:py-[100px] z-10 relative justify-center min-[992px]:py-40 '
    >
      <GridWrapper>
